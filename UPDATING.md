...
- **Removed deprecated `AVOID_COLORS_COLLISION` feature flag.** The flag defaulted to `True` and its color-collision-avoidance logic in `CategoricalColorScale` is now always applied. Remove any `AVOID_COLORS_COLLISION` entry from your `FEATURE_FLAGS` config; setting it has no effect. Deployments that had explicitly disabled it (`AVOID_COLORS_COLLISION = False`) will now always get distinct-color reassignment for colliding chart series.
